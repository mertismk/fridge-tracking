...
def test_user_model(db_session):
    ...
    ), "Имя пользователя должно совпадать с заданным"
    assert (
        user.email == "test_user@example.com"
    ), "Email должен совпадать с заданным"
    assert user.check_password(
        "password"
    ), "Проверка пароля должна возвращать True"
    assert not user.check_password(
        "wrong_password"
    ), "Проверка неверного пароля должна возвращать False"

    assert "User" in str(user), "__repr__ должен содержать класс"
    assert "test_user" in str(
        user
    ), "__repr__ должен содержать имя пользователя"
...
def test_product_model(db_session):
    ...
    ), "Название продукта должно совпадать с заданным"
    assert (
        product.category == "Тестовая категория"
    ), "Категория должна совпадать с заданной"
    assert product.quantity == 1.5, "Количество должно совпадать с заданным"
    assert (
        product.unit == "кг"
    ), "Единица измерения должна совпадать с заданной"

    assert (
        product.expiry_date.astimezone(timezone.utc).date()
        == expiry_date.date()
    ), "Срок годности должен совпадать с заданным"

    assert (
        not product.is_expired()
    ), "Новый продукт не должен быть просроченным"

    assert (
        4 <= product.days_until_expiry() <= 5
    ), "Срок до истечения должен быть около 5 дней"

    assert "Product" in str(product), "__repr__ должен содержать класс"
    assert "Тестовый продукт" in str(
        product
    ), "__repr__ должен содержать название продукта"
...
def test_shopping_item_model(db_session):
    user = User(
        username="shopping_test_user", email="shopping_test@example.com"
    )
    user.set_password("password")
    db_session.add(user)
    db_session.commit()
    ...
    assert (
        item.name == "Тестовый элемент"
    ), "Название элемента должно совпадать с заданным"
    assert (
        item.category == "Тестовая категория"
    ), "Категория должна совпадать с заданной"
    ...
    assert (
        item.is_purchased is False
    ), "Статус покупки должен совпадать с заданным"
    assert (
        item.user_id == user.id
    ), "ID пользователя должен совпадать с заданным"

    assert "ShoppingItem" in str(item), "__repr__ должен содержать класс"
    assert "Тестовый элемент" in str(
        item
    ), "__repr__ должен содержать название элемента"
